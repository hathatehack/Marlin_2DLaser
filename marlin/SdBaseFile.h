...
class SdBaseFile {
 public:
   ...
   */
  void getpos(fpos* pos);
  /** set position for streams
   * \param[out] pos struct with value for new position
   */
  void setpos(fpos* pos);
  //----------------------------------------------------------------------------
  bool close();
  ...
  void setvolume(SdVolume* volume) {vol_ = volume;}
  uint8_t flags(void) {return flags_;}
  void setflags(uint8_t flags) {flags_ = flags;}
  void setvolcacheBlockNumberDirty(uint32_t blockNumber, bool dirty) {vol_->cacheBlockNumber_  = blockNumber; vol_->cacheDirty_ = dirty;}
  void setfileSize(uint32_t fileSize) {fileSize_ = fileSize;}
