...
/// Read a pin
#define _READ(IO) ((bool)(DIO ## IO ## _PORT->IDR & DIO ## IO ## _PIN ? 1 : 0))
//#define _READ(IO) ((bool)digitalRead(IO))

#define ___IN(PORT, PIN) ((bool)P##PORT##in(PIN))
#define __IN(PORT, PIN) ___IN(PORT, PIN)
#define _IN(IO) __IN(BB ## IO ## _PORT, BB ## IO ## _PIN)
/// write to a pin
// On some boards pins > 0x100 are used. These are not converted to atomic actions. An critical section is needed.

#define _WRITE(IO, v)  do {  if (v) {DIO ##  IO ## _PORT->BSRR = DIO ## IO ## _PIN; } else {DIO ##  IO ## _PORT->BRR = DIO ## IO ## _PIN; } } while (0)
//#define _WRITE(IO, v) digitalWrite(IO, v)

#define ___OUT(PORT, PIN, v) do{P##PORT##out(PIN) = v;} while(0)
#define __OUT(PORT, PIN, v) ___OUT(PORT, PIN, v)
#define _OUT(IO, v) __OUT(BB ## IO ## _PORT, BB ## IO ## _PIN, v)

/// toggle a pin
#define _TOGGLE(IO)  do {_WRITE(IO, !_READ(IO)); } while (0)

/// set pin as input
//#define	_SET_INPUT(IO, PERIPH, PULL) do {RCC->APB2ENR |= RCC_APB2Periph ## _PERIPH; \
//                                DIO ## IO ## _PORT->DIO ## IO ## _DDR &= ~(uint32_t)(0x0F) << (DIO ##  IO ## _POS << 2); \
//                               DIO ## IO ## _PORT->DIO ## IO ## _DDR |= (uint32_t)(PULL & 0x0F) << (DIO ##  IO ## _POS << 2); } while (0)
#define	_SET_INPUT(IO, PULL) pinMode(IO, PULL)

/// set pin as output
//#define	_SET_OUTPUT(IO, PERIPH) do {RCC->APB2ENR |= RCC_APB2Periph ## _PERIPH; \
//                            DIO ## IO ## _PORT->DIO ## IO ## _DDR &= ~(uint32_t)(0x0F) << (DIO ##  IO ## _POS << 2); \
//                            DIO ## IO ## _PORT->DIO ## IO ## _DDR |= (uint32_t)(GPIO_Mode_Out_PP & 0x0F | GPIO_Speed_50MHz) << (DIO ##  IO ## _POS << 2); } while (0)
...
//#define SET_INPUT(IO, PULL)  _SET_INPUT(IO, DIO ## IO ## _PORT, PULL)
#define SET_INPUT(IO, PULL)  _SET_INPUT(IO, PULL)
/// set pin as output wrapper
//#define SET_OUTPUT(IO)  _SET_OUTPUT(IO, DIO ## IO ## _PORT)
...
//static inline unsigned short _analogRead(void)
//{
//  ADC1->CR2 |= 0x500000;
//  while(!(ADC1->SR & 0x02));
//  return (ADC1->DR >> 2);
//}
static inline unsigned short _analogRead(unsigned char ADC_Channel)
{
//  ADC1->SMPR2 &= ~0x07 << (3 * ADC_Channel);
//  ADC1->SMPR2 |= 0x05 << (3 * ADC_Channel);
  ADC1->SQR3 &= ~0x1F;
...
#if defined (ARDUINO_STM32_F103VB)
/*
pins
*/
...
#define DIO6_PORT      GPIOB
#define DIO6_DDR       CRL
#define DIO6_POS       5

#define DIO7_PIN       GPIO_Pin_6
#define DIO7_PORT      GPIOB
#define DIO7_DDR       CRL
#define DIO7_POS       6

#define DIO8_PIN       GPIO_Pin_7
#define DIO8_PORT      GPIOB
#define DIO8_DDR       CRL
#define DIO8_POS       7

#define DIO9_PIN       GPIO_Pin_8
#define DIO9_PORT      GPIOB
#define DIO9_DDR       CRH
#define DIO9_POS       0

#define DIO10_PIN       GPIO_Pin_9
#define DIO10_PORT      GPIOB
#define DIO10_DDR       CRH
#define DIO10_POS       1

#define DIO12_PIN       GPIO_Pin_5
#define DIO12_PORT      GPIOC
#define DIO12_DDR       CRL
#define DIO12_POS       5
